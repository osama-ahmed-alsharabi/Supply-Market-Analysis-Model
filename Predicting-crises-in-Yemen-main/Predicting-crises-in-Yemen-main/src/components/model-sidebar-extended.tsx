...
import { ChevronLeft, ChevronRight, Server, Clock, Truck, Radio, Utensils, Activity, Settings, FileText, Database, HelpCircle, LayoutDashboard, BarChart3 } from 'lucide-react';
import { Badge } from '@/components/ui/badge';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { DashboardContext, ActiveView } from '@/components/dashboard-client-extended';
...
export function ModelSidebarExtended() {
    ...
    const { activeView, setActiveView, t, language } = React.useContext(DashboardContext);
            ...
            <ScrollArea className="flex-1">
                {isOpen && (
                    <div className="p-4 space-y-4">
                        ...
                        {/* ========== NEW: SUPPLY & MARKET ANALYSIS BUTTON ========== */}
                        <Button
                            variant={activeView === 'supply-market' ? "default" : "outline"}
                            className={cn(
                                "w-full justify-start gap-3 h-14 transition-all mb-3",
                                activeView === 'supply-market'
                                    ? "bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-700 hover:to-emerald-800 border-0"
                                    : "bg-gradient-to-r from-emerald-500/10 to-emerald-500/5 border-emerald-500/30 hover:bg-emerald-500/20"
                            )}
                            onClick={() => setActiveView(activeView === 'supply-market' ? 'dashboard' : 'supply-market')}
                        >
                            <div className={cn(
                                "p-2 rounded-lg",
                                activeView === 'supply-market' ? "bg-white/20" : "bg-emerald-500/20"
                            )}>
                                <BarChart3 className={cn("h-5 w-5", activeView === 'supply-market' ? "text-white" : "text-emerald-500")} />
                            </div>
                            <div className="text-left">
                                <div className={cn("font-semibold text-sm", activeView === 'supply-market' && "text-white")}>
                                    📊 {language === 'ar' ? 'تحليل العرض والسوق' : 'Supply & Market'}
                                </div>
                                <div className={cn("text-[10px]", activeView === 'supply-market' ? "text-white/70" : "text-muted-foreground")}>
                                    {activeView === 'supply-market'
                                        ? (language === 'ar' ? 'الحالة: نشط' : t.status + ": Active")
                                        : (language === 'ar' ? 'توقع التكاليف والأسواق' : 'Cost & Market Forecasting')}
                                </div>
                            </div>
                            {activeView === 'supply-market' && (
                                <Radio className="h-4 w-4 text-white ml-auto animate-pulse" />
                        ...
                        {/* General Factors REMOVED - replaced by Supply & Market Analysis */}
                ...
                {!isOpen && (
                    <div className="p-2 space-y-2">
                        ...
                        {/* NEW: Supply Market Icon */}
                        <Button
                            variant={activeView === 'supply-market' ? "default" : "ghost"}
                            size="icon"
                            className={cn("w-10 h-10", activeView === 'supply-market' && "bg-emerald-600 hover:bg-emerald-700")}
                            onClick={() => setActiveView(activeView === 'supply-market' ? 'dashboard' : 'supply-market')}
                            title={language === 'ar' ? 'تحليل العرض والسوق' : 'Supply & Market'}
                        >
