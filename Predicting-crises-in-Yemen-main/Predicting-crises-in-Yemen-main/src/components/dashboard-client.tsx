...
export type ActiveView = 'dashboard' | 'supply-chain' | 'food-security' | 'settings' | 'reports' | 'data' | 'help';
...
export function DashboardClient() {
    ...
    // Update Reports when Language Changes (to simulate localized data)
    React.useEffect(() => {
        setReports([
    ...
    const handleRunPrediction = async () => {
        ...
        if (result.error || !result.data) {
            ...
            // When prediction is successful, the system creates a report
    ...
    const handleSendMessage = async (message: string) => {
        // Allow chat when showing supply chain or when prediction data exists
        if (!predictionData && !showSupplyChain) return;

        const newMessages: ChatMessage[] = [...chatMessages, { role: 'user', content: message }];
        setChatMessages(newMessages);
        setIsSendingMessage(true);

        // Use prediction data or supply chain context
        const context = predictionData?.statistics || JSON.stringify({
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="hidden md:flex items-center gap-1">
                                {activeView === 'supply-chain' && (
                                    <span className="text-sm text-muted-foreground">• {t.supplyChain}</span>
                                )}
                                {activeView !== 'dashboard' && activeView !== 'supply-chain' && (
                                    <span className="text-sm text-muted-foreground capitalize">• {
                                        activeView === 'settings' ? t.settings :
                                            activeView === 'reports' ? t.reports :
                                                activeView === 'data' ? t.dataConnectors :
                                                    activeView === 'help' ? t.helpDocs : activeView
                                    }</span>
                                )}
                            </div>
                        </div>
                    ...
                    `}>
                        <div className="h-full overflow-hidden relative">
                            <ChatPanel
                                ...
                                isChatEnabled={!!predictionData || showSupplyChain}
                            />
